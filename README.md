...
工作 / …   （空，等第一篇）
生活 / 猜灯谜 · 八段锦 · 数独 · 象棋 · 五子棋 · 书法                          （6 篇，每项一个二级分类）
申论知识库、言语理解（等新母本，导入时照这条来，一级挂「学习」）
```

- **三层是「学习」的口径，不是全站的死规矩**。「生活」这一级用户 2026-09-24 定的是**两项分类当两层用**（`生活 / 猜灯谜`），因为六套课一项一篇，再切第三层就是每层一篇、等于没有分类。`check-build.py` 第 1b 节按这个放行：一级必须是 工作/学习/生活，层数 2 或 3，**只有「学习」必须满三层**。
...
- **一个文件一篇**最省事。一个文件里塞多篇（一整册、一个月）就得写切块脚本，切到哪一层由分类需要决定——时政按「一条考点一篇」、行测按「一个难题组一篇」，都是为了分类页能归类；素描反过来，母本 31 个文件**合成 7 篇**（一个阶段一篇），因为用户要的是一篇里装完一个大模块、后续只在原篇里更新；生活六项更彻底，**64 个课文件合成 6 篇**（一项一篇）——同一句话里用户要的既是「别太多篇」也是「一门爱好好读」，别拿「一个 MD 一篇」的默认约定去套。
...
模块名：行测常识
母本位置：D:\AI跑数据\学习项目\...
分类：生活 / 猜灯谜   ← 写到你想要的层数（1~3 层）；「学习」是三层，「生活」两层，别的模块层数不够或想压层我会先问
...
- **52 周热力图**（整行一张卡）：按 `p.date.format('YYYY-MM-DD')` 分组，格子从「本周所在周的周一」往前推
  ...
  `2025-11-01 · 17 篇 · 时政要点`。日期上没写文章的一律是浅色 —— 现在 125 篇只落在 9 个日期上，
...
「学习」卡上的题数文案和二级分类芯片与源现算结果一致（芯片按一级模块分开比，「生活」卡单独查那 6 个二级分类，都是从 `source/_posts` 现算）、首页每个 href/src 都落盘、
最近更新 == 全站最新 6 篇、`public/page/2/` 还是文章流、每篇行测「折叠块数 == 答案数」且速览恰好 1 块；
第 10 节查生活六项：每篇的 `<h2>` 课数 == manifest 课数、课标题逐条在渲染页里对得上、四处图片集合（母本 svg / 源 md 引用 / `source/images` / `public/images`）完全相等、单页汉字 > 2 万、合计 64 课 499 图、站内链接无死链。
...
## 生活六项导入流水线（6 篇 / 64 课 / 499 图）

2026-09-24 用户：「将我的：猜灯谜，八段锦，数独，象棋，五子棋，书法，按上面要求push上去」，随后定了两件事——分类 **生活 / 六项（当二级用）**、粒度 **一项一篇，先上线**。母本就是他自己的另一套课程产物（和素描同源不同项目）：
...
D:\blog\source\_posts\生活六项\sh-*.md（6 篇）   +   source\images\shenghuo\<项>\*.svg（499 张，母本全部）
```

| 项 | slug | 课 | 图 | 正文 |
|---|---|---|---|---|
| 猜灯谜 | `sh-riddle` | 11 | 81 | ~3.0 万汉字 |
| 八段锦 | `sh-baduanjin` | 10 | 79 | ~3.1 万 |
| 数独 | `sh-sudoku` | 11 | 81 | ~3.8 万 |
| 象棋 | `sh-xiangqi` | 11 | 86 | ~4.1 万 |
| 五子棋 | `sh-gomoku` | 10 | 70 | ~2.9 万 |
| 书法 | `sh-calligraphy` | 11 | 102 | ~5.1 万 |

分类二级名字用用户写的原词（**「象棋」不是「中国象棋」**，标题里才是全名）；一项一篇所以每篇只有一个二级分类、里面 10~11 个 `## NN 课名`，文章左侧「文章目录」按课跳（NexT 的选择器还是 `.post-toc .nav-link`，见素描那节末尾的提醒）。日期 `2026-09-24 09:0N:00` 按六项顺序递增，前台「最近更新」就是书法→象棋→…的倒序。

**结构和素描那个脚本是同一套**：读每项的 `course.manifest.js` 拿「阶段↔课」对应（`module.exports = ({…})` 用 `new Function('return …')()` 求值，manifest 和 MD 数量、标题对不上就直接 die），课文件名按 `` `${l.n}-${basename(l.file,'.html')}.md` `` 拼；每篇开头自动生成「这套课：…N 个阶段共 X 课、约 Y 万汉字、Z 张图」+ 一张 `课 / 阶段 / 标题 / 这一课干什么` 的总表，然后逐课接正文。三处允许的改写和素描完全一样（软换行合并、`**x**` → `<strong>`、图片改绝对路径 `/images/shenghuo/<项>/<文件名>`），另外多做一件：**课内标题整体降一级**（母本的 `##` 变 `###`），把 `##` 留给课号，否则一层的目录里 300 个标题挤在一起。

**两道自检**：① 逐行——每项每一课除 H1/H2 外每一行（同规则归一后）必须在生成的那篇里原样出现；② 结构——64 课全部入篇、每篇 `## NN ` 数 == manifest 课数、`###` 数 == 母本这些课的 `##` 数、引用图片全部落盘、无 `**` 残留、无 `](images/` 相对路径、母本 svg 一张不漏（孤儿数按 `<项>/<文件名>` 判，不同项同名 svg 不会互相顶掉）。
...
**代价要说清楚**：一项一篇 = 单页很重。最重的象棋一页 HTML 约 277KB（`public` 里那个文件），86 张 SVG 合计 344KB，本地一次点亮全部图约 0.45 秒；`public/` 整站从 ~44MB 涨到 57MB。手机上首屏没问题（图是矢量、按需解码），但**微信里翻到中间会感觉到长**。真要拆，`import-shenghuo.mjs` 里把「一项一篇」改成「一个阶段一篇」就行（`TOPICS` 展开成阶段循环，其余不用动），代价是分类页会出现三层、和「生活只写两级」的口径冲突，得重新问用户。
