...
D:\blog\source\_posts\行测题库\<模块>\xc-*.md   +   source\images\xingce\<ascii>\*.png（只搬引用到的 84 张）
```
...
已知缺口（2026-09-22 首导，都是母本本身的问题，脚本不猜）：

- **言语理解下册只有 1 组**：答案键里有 `语句表达(二)(三)`、`中心理解(一)~(四)` 的答案，母本 md 里没有这些题。答案键那篇文章末尾列了这份对账。
- **数量关系缺图**：上册 1 处、下册 5 处「如下图所示」但整篇无图；`assets/数量上`(9)、`assets/数量下`(8)、`assets/判断上`(24) 三个目录共 41 张图 md 一处都没引用，大概率就是丢掉的配图。`xc-pd1-03` 里还留着「（图片选项，原讲义 p17 为花序示意图）」这样的占位文字。按题号配图要靠人对页码，我不敢猜，等你确认。
