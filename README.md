...
母本结构已摸清（`### 🎯 第N题` 之外没有任何标题，整篇就一个 H1）：

- **982 个题块，798 个带【解析】，184 个没有**（另有 1 个题块里塞了两条解析）。缺的要如实报出来，别默认「没解析 = 错题」。
- **题号不能当标识**：`第N题` 在 75 处从 N 重置回 1——这是多套真题拼在一起的，每套自己从 1 编号。唯一可靠的是全局序号（第几块），或者题面第一行的来源标注 `1.（2024江苏A29）`——982 题全都有这个标注，所以「按真题来源」分类是可做的，「按考点」得另外判断（母本没给）。

导入前先定分类：按真题来源（省份/年份，标注里现成）、按考点（要靠 `言语理解_按知识点分类.md` 那份 79 题的映射，覆盖不全），还是像行测那样按区间切篇。行测的「一个难题组一篇」在这里不成立，母本没有组标题。
