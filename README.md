...
还有一张手绘的课程树 `/sketch-map/`（源文件 `source/sketch-map/index.md`），由 `import-sketch.mjs` 按 manifest 生成（三大类 → 6 阶段 → 30 课，每格链到该课的锚点，check-build 第 8 节逐条核锚点存在）。**2026-09-23 用户说「素描课程不要」，所以顶部菜单项和首页模块卡上的总览链接都撤了**（那会儿素描还挂在「学习」卡下，现在整个模块在「生活」）；页面本身和 7 篇素描文章保留，只能靠直接网址或分类页 `/categories/生活/素描/` 进去——2026-09-24 素描换一级之后，`/sketch-map/` 这个地址本身没变（它不是文章，不受 permalink 影响），但 `/categories/学习/素描/…` 那一串旧地址全成 404 了。以后要恢复入口：`_config.next.yml` 的 `menu` 加回 `素描课程: /sketch-map/ || fa fa-pencil-alt`；想在首页卡上再放一个下钻链接，就在 `TOPS` 里给那个模块补 `href` / `hrefTxt`，并在模板 `tools/bento-home.njk` 的模块卡循环里加回 `{% if m.href %}<a class="bento-more" …>{% endif %}`（2026-09-23 把没人用的那段删了）。
...
- **52 周热力图**（整行一张卡）：按 `p.date.format('YYYY-MM-DD')` 分组，格子从「本周所在周的周一」往前推
  ...
  `2025-11-01 · 17 篇 · 时政要点`。日期上没写文章的一律是浅色 —— 现在 137 篇只落在 9 个日期上，
