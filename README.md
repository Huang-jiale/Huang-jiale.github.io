...
- **一个文件一篇**，不要在一个文件里塞多篇再用标题分隔（那样就得写切块脚本）。
...
1. 抽查 2~3 篇渲染后的正文：有没有露出的 `**`、`<br>` 把句子切碎、正文比母本少一段。
2. 分类页：`/categories/<模块>/<子分类>/` 打开，条数对不对、分页能翻到最后一项。
3. 引用：文内链接的目标页 `curl` 一下是不是 200（中文路径要百分号编码）。
4. 老 URL 是否如预期消失或保留。
5. 自查脚本要先断言「采样数量 == 文章数」再报结论——正则写错导致匹配到 0 个文件时，所有检查都会假绿。
