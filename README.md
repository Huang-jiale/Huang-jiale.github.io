...
**课程总览页 `source/sketch-map/index.md`**（URL `/sketch-map/`，顶部菜单「素描课程」）也是这个脚本生成的：三大类各一节，每个阶段一行，行末把该阶段的课全部横着排开，每格链到那一课的标题锚点（`/2026/09/23/sk-stage2/#08-圆柱、圆锥与椭圆的秘密`）。锚点规则是 Hexo 给标题加的 id：原文空格换成 `-`，中文和标点原样保留（`check-build.py` 第 8 节会逐个回查这 30 个锚点在目标页里真的存在，比对前先 unquote，因为 Hexo 输出时把中文锚点百分号编码了）。母本改了课程数，这一页跟着重跑就行。
