...
切法：**一个阶段一篇**（用户 2026-09-23：按大模块分类、不要太多篇、方便后面更新）——`sk-stage1..sk-stage6` + `sk-intro` 共 7 篇，单篇 10~74KB。分类两级 `素描 / 阶段N 名称`（如 `素描 / 阶段六 创作与精进`，`sk-intro` 挂 `素描 / 入门总览`），每篇正文用 `## NN 课名` 保住母本的课边界（阶段一 5 课 → 二 5 → 三 6 → 四 4 → 五 6 → 六 4，合计 30），页内目录就按课跳。日期错开在 `2026-09-23 12:0N:00`，顺序即阶段顺序；`sk-intro` 单独 12:10:00 排最后。
