...
两个坑：① `**粗体**` 夹在中文字符里会被 CommonMark 判成非边界（`纳入**「四个全面」战略布局**` 会把星号原样印到页面上），生成时统一换成 `<strong>` 标签——母本 391 对 `**` 全部同行成对，替换无损。② 母本里三处「详见本页【专题一】」依赖的是同一份文档，拆成 6 篇后就成了失效引用，`xref()` 把它们换成指向专题篇的绝对链接；这是唯一允许的文字改写，自检两侧套用同一规则，所以别处丢了照样报。
